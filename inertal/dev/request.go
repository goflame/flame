...
func (*Request) Log(m string, p string) {
	...
	tColor := color.New(color.FgHiBlack).Sprintf("%v", t)
	if err != nil || l <= 0 {
		fmt.Printf("[%v] %v %v %v\n", method, tColor, color.New(color.FgHiBlack).Sprintf("%v", ". . ."), p)
	} else {
		dot := ". "
		dots := strings.Repeat(dot, int(math.Round(float64(l/2))))
		fmt.Printf("[ %v ] %v %v%v\n", method, tColor, color.New(color.FgHiBlack).Sprintf("%v", dots), p)
